...
import { demoQuestions } from "../DemoAppStore";
      ...
      <main className="h-full max-w-md flex flex-col items-center gap-10 p-6">
        ...
        <div className="relative max-w-9/10">
          <div aria-live="polite" aria-atomic="true">
            <QuestionCard question={demoQuestions[current].question} />
          </div>
          ...
          <div className="absolute top-1/2 -translate-y-1/2 -left-6 -right-6 flex justify-between">
            ...
            <NavButton
              label="Next question"
              icon={ArrowRightIcon}
              onClick={() => setCurrent((c) => Math.min(demoQuestions.length - 1, c + 1))}
            />
          </div>
        </div>
        ...
        <Button
          onClick={onNext}
          className={current === demoQuestions.length - 1 ? "" : "invisible pointer-events-none"}
        >
          We are finished
        </Button>
      </main>
